...
from hyurl.tools.gae import calculate_gae
...
class Memory:
    ...
    def store(self, s, a, r, done, log_prob, mask, value, logit):
        ...
        if done or len(self.fragment.actions) >= 128:
            states, actions, logits, masks, logps, values, dones, advs = (
                self.fragment.gens()
            )
    ...
    def fstore(self, fragment: Fragment):
        ...
        def trans(lis, dic):
            for ilis in lis:
                if not isinstance(ilis, dict):
                    raise ValueError(f"not dict: {ilis}")
                for k, v in ilis.items():
                    if k not in dic:
                        dic[k] = []
                    dic[k].append(v)
